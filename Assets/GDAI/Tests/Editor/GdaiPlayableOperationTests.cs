...
namespace GDAI.Bridge.Editor.Tests
{
    public class GdaiPlayableOperationTests
    {
        ...
        public void Resume_VerifiesProjectTokenSnapshotContract()
        {
            ...
            // happy path
            Assert.AreEqual(GdaiPlayableOperation.ResumeVerdict.Resumable, op.CanResume(PID, SNAP, CVER, true, out _));
            // each mismatch is refused
            Assert.AreEqual(GdaiPlayableOperation.ResumeVerdict.Unsafe, op.CanResume(PID, SNAP, CVER, false, out _), "no token");
            Assert.AreEqual(GdaiPlayableOperation.ResumeVerdict.Unsafe, op.CanResume("other", SNAP, CVER, true, out _), "project mismatch");
            Assert.AreEqual(GdaiPlayableOperation.ResumeVerdict.Unsafe, op.CanResume(PID, "changed", CVER, true, out _), "snapshot changed");
            Assert.AreEqual(GdaiPlayableOperation.ResumeVerdict.Unsafe, op.CanResume(PID, SNAP, "v2/other", true, out _), "contract revision changed");
        }
        ...
        public void Terminal_OperationsAreNotActive()
        {
            ...
            Assert.IsNull(active);
            Assert.AreEqual(GdaiPlayableOperation.ResumeVerdict.Terminal, op.CanResume(PID, SNAP, CVER, true, out _));
        }
